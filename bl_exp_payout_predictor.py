...
class BLPayoutModelsPredict:
    ...
    def load_models(self):
        tfm_gen = joblib.load(self.predictors_path + 'payout_tfm_generator.joblib')
        CB_bl_lead = CatBoostClassifier(allow_writing_files=False).load_model(
            self.predictors_path + 'CB_bl_lead.cbm', format='cbm')
        # Load the model from the file
        with open(self.predictors_path + 'LGBM_bl_payout.pkl', 'rb') as f:
            LGBM_bl_payout = pickle.load(f)
        return tfm_gen, LGBM_bl_payout, CB_bl_lead
    ...
    def prediction_expected_payout(self, bl_data, model_tfm, LGBM_bl_payout, CB_bl_lead):
        cat_features = bl_data.select_dtypes(include=['object', 'category']).columns.tolist()
        for col in cat_features:
            bl_data[col] = bl_data[col].astype('category')
        self.logger.info('start tabular transformer feature calculation')
        bl_data['tfm_payout_pred'] = model_tfm.predict(bl_data)
        self.logger.info('finish tabular transformer feature calculation')
        bl_data['payout'] = np.exp(LGBM_bl_payout.predict(bl_data))
        bl_data['predicted_prob_lead'] = CB_bl_lead.predict_proba(bl_data)[:, 1]
    ...
    def predict_(self):
        ...
        tfm_gen, LGBM_bl_payout, CB_bl_lead = self.load_models()
        self.logger.info('models were loaded')
        result_clients_ranks = self.prediction_expected_payout(bl_data, tfm_gen, LGBM_bl_payout, CB_bl_lead)
...
if __name__ == "__main__":
    ...
    user_data_path = '/Users/yurygubman/Data/BL/'
