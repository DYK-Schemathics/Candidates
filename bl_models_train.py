...
class BLPayoutModelsFit:
    ...
    def feature_from_tabular_transformer(self, x_train_payout, y_train_payout):
        self.logger.info('start tabular transformer')
        CONTEXT_SIZE = 1000 # most recent context
        model_tfm = TabPFNRegressor(device='cpu', ignore_pretraining_limits=True)
        model_tfm.fit(x_train_payout.iloc[-CONTEXT_SIZE:], y_train_payout['payout'].iloc[-CONTEXT_SIZE:])
        self.logger.info('finish tabular transformer')
        x_train_payout['tfm_payout_pred'] = model_tfm.predict(x_train_payout)
        return x_train_payout, model_tfm

    def lightgbm_regression_payout(self, x_train_payout, y_train_payout):
        cat_features = x_train_payout.select_dtypes(include=['object', 'category']).columns.tolist()
        for col in cat_features:
            x_train_payout[col] = x_train_payout[col].astype('category')
        y_train_log_payout = np.log1p(y_train_payout['payout'])
        LGB_bl_payout = LGBMRegressor(
            random_state=42,
            max_depth=7,
            n_estimators=800,
            learning_rate=0.03,
        ...
        LGB_bl_payout.fit(x_train_payout, y_train_log_payout, categorical_feature=cat_features)
        self.logger.info('finish lightgbm_regression_payout fit')
        return LGB_bl_payout

    def accuracy_cont_payout_prediction(self, LGBM_bl_payout, model_tfm, x_test_payout, y_test_payout):
        x_test_payout['tfm_payout_pred'] = model_tfm.predict(x_test_payout)
        cat_features = x_test_payout.select_dtypes(include=['object', 'category']).columns.tolist()
        for col in cat_features:
            x_test_payout[col] = x_test_payout[col].astype('category')

        y_test_payout['pred_payout'] = np.exp(LGBM_bl_payout.predict(x_test_payout))
    ...
    def save_models(self, model_tfm, CB_bl_lead, LGBM_bl_payout):
        joblib.dump(model_tfm, self.output_predictors_path + 'payout_tfm_generator.joblib')
        with open(self.output_predictors_path + 'LGBM_bl_payout.pkl', 'wb') as f:
            pickle.dump(LGBM_bl_payout, f)
    ...
    def fit_(self):
        ...
        x_train_payout, model_tfm = self.feature_from_tabular_transformer(x_train_payout, y_train_payout)
        LGBM_bl_payout = self.lightgbm_regression_payout(x_train_payout, y_train_payout)
        self.logger.info("LightGMB regressor for $ payout finished")
        ...
        if self.train_test == True:
            ...
            self.accuracy_cont_payout_prediction(LGBM_bl_payout, model_tfm, x_test_payout, y_test_payout)
            self.logger.info("Train-test assessment finished")
        else:
            self.save_models(model_tfm, CB_bl_lead, LGBM_bl_payout)
            self.logger.info("Train on all data for production finished")


if __name__ == "__main__":
    input_path = '/Users/yurygubman/Data/BL/'
